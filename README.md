...
依頼の日別指定は `3 + 8 + 8 + 2 = 21回` で、別途記載された「合計20回」と算術上一致しません。本実装は、より具体的な日別指定を優先して次の21回で生成済みです。
...
必ず20回にする場合は、本番開始前にどの日を1回減らすかを決め、[`config/production.json`](config/production.json) の `count` を変更して「[スケジュールの作り直し](#スケジュールの作り直し)」を実行してください。
...
- [ ] 日別指定を優先した21回でよいか、合計20回へ修正するか決めた
